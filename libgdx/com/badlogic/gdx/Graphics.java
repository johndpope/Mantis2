...
public interface Graphics {
	 ...
	 * instance of {@link GL30} via {@link #getGL30()} to access OpenGL ES 3.0
	 * functionality. Note that this functionality will only be available if you
	 * instructed the {@link Application} instance to use OpenGL ES 3.0!
	 *
	...
	/**
	 * Returns the id of the current frame. The general contract of this method
	 * is that the id is incremented only when the application is in the running
	 * state right before calling the {@link ApplicationListener#render()}
	 * method. Also, the id of the first frame is 0; the id of subsequent frames
	 * is guaranteed to take increasing values for 2<sup>63</sup>-1 rendering
	 * cycles.
	 * 
	...
	/**
	 * @return the supported fullscreen {@link DisplayMode}(s) of the monitor
	 *         the window is on
	 */
	 ...
	 * implementation. It may be necessary for the window to be recreated in
	 * order for the changes to take effect. Consult the documentation for the
	 * backend in use for more information.
	 *
	 * Supported on all GDX desktop backends and on Android (to disable the menu
	 * bar).
	 *
	 * @param undecorated
	 *            true if the window border or status bar should be hidden.
	 *            false otherwise.
	 */
	...
	/**
	 * Requests a new frame to be rendered if the rendering mode is
	 * non-continuous. This method can be called from any thread.
	 */
	 ...
	 * necessarily equal) and of a certain minimum size (32x32 is a safe bet),
	 * and alpha transparency must be single-bit (i.e., 0x00 or 0xFF only). This
	 * function returns a Cursor object that can be set as the system cursor by
	 * calling {@link #setCursor(Cursor)} .
	 *
	 * @param pixmap
	 *            the mouse cursor image as a
	 *            {@link com.badlogic.gdx.graphics.Pixmap}
	 * @param xHotspot
	 *            the x location of the hotspot pixel within the cursor image
	 *            (origin top-left corner)
	 * @param yHotspot
	 *            the y location of the hotspot pixel within the cursor image
	 *            (origin top-left corner)
	 ...
	 * support cursor:url() and support the png format (the pixmap is converted
	 * to a data-url of type image/png) should also support custom cursors. Will
	 * set the mouse cursor image to the image represented by the

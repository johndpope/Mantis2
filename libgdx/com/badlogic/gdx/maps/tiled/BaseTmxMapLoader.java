...
public abstract class BaseTmxMapLoader<P extends AssetLoaderParameters<TiledMap>>
	...
	public static class Parameters extends AssetLoaderParameters<TiledMap> {
		...
		/**
		 * Whether to flip all Y coordinates so that Y positive is down. All
		 * LibGDX renderers require flipped Y coordinates, and thus flipY set to
		 * true. This parameter is included for non-rendering related purposes
		 * of TMX files, or custom renderers.
		 */
	...
	protected void loadObject(TiledMap map, MapLayer layer, Element element) {
		if (element.getName().equals("object")) {
			...
			float y = (flipY ? (mapHeightInPixels - element.getFloatAttribute("y", 0))

...
public class Group extends Actor implements Cullable {
	 ...
	 * {@link #drawChildren(Batch, float)}, then {@link #resetTransform(Batch)}
	 * if needed.
	 */
	 ...
	 * called before and {@link #resetTransform(Batch)} after this method if
	 * {@link #setTransform(boolean) transform} is true. If
	 * {@link #setTransform(boolean) transform} is false these methods don't
	 * need to be called, children positions are temporarily offset by the group
	 * position when drawn. This method avoids drawing children completely
	 * outside the {@link #setCullingArea(Rectangle) culling area}, if set.
	 */
	 ...
	 * flushed. {@link #resetTransform(ShapeRenderer)} will restore the
	 * transform to what it was before this call.
	 */
	 ...
	 * {@link #applyTransform(Batch, Matrix4)}. Note this causes the shape
	 * renderer to be flushed.
	 */
	...
	/**
	 * Children completely outside of this rectangle will not be drawn. This is
	 * only valid for use with unrotated and unscaled actors.
	 * 
	 * @param cullingArea
	 *            May be null.
	 */
	 ...
	 * parent. If the actor is already a child of this group, no changes are
	 * made.
	 */
	...
	/**
	 * Adds an actor as a child of this group at a specific index, removing it
	 * from its previous parent. If the actor is already a child of this group,
	 * no changes are made.
	 * 
	 * @param index
	...
	/**
	 * Adds an actor as a child of this group immediately after another child
	 * actor, removing it from its previous parent. If the actor is already a
	 * child of this group, no changes are made.
	 */
	 ...
	 * has actions, they should be {@link Actor#clearActions() cleared} so the
	...
	protected void setStage(Stage stage) {
		...
		Actor[] childrenArray = children.items;
		for (int i = 0, n = children.size; i < n; i++)
			childrenArray[i].setStage(stage); // StackOverflowError here means
												// the group is its own
												// ancestor.
	}
	...
	/**
	 * Swaps two actors. Returns false if the swap did not occur because the
	 * actors are not children of this group.
	 */
	 ...
	 * in their parent's coordinate system. This has a performance impact
	 * because {@link Batch#flush()} must be done before and after the
	 * transform. If the actors in a group are not rotated or scaled, then the
	 * transform for the group can be set to false. In this case, each child's
	 * position will be offset by the group's position for drawing, causing the
	 * children to appear in the correct location even though the Batch has not
	 * been transformed.
	 */
	...
	/**
	 * If true, {@link #drawDebug(ShapeRenderer)} will be called for this group
	 * and, optionally, all children recursively.
	 */

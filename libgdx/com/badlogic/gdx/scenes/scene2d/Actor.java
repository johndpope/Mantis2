...
public class Actor {
	 ...
	 * This draw method} is convenient to draw a rotated and scaled
	 * TextureRegion. {@link Batch#begin()} has already been called on the
	 * batch. If {@link Batch#end()} is called to draw without the batch then
	 ...
	 * propagates the event to this actor and ancestor actors as necessary. If
	 * this actor is not in the stage, the stage must be set before calling this
	 * method.
	 * <p>
	 * Events are fired in 2 phases:
	 * <ol>
	 * <li>The first phase (the "capture" phase) notifies listeners on each
	 * actor starting at the root and propagating downward to (and including)
	 * this actor.</li>
	 * <li>The second phase notifies listeners on each actor starting at this
	 * actor and, if {@link Event#getBubbles()} is true, propagating upward to
	 * the root.</li>
	 * </ol>
	...
	public boolean fire(Event event) {
		...
		// Collect ancestors so event propagation is unaffected by hierarchy
		// changes.
		...
		try {
			...
			// Notify all parent listeners, starting at the target. Children may
			// stop an event before ancestors receive it.
	...
	/**
	 * Notifies this actor's listeners of the event. The event is not propagated
	 * to any parents. Before notifying the listeners, this actor is set as the
	 * {@link Event#getListenerActor() listener actor}. The event
	 * {@link Event#setTarget(Actor) target} must be set before calling this
	 * method. If this actor is not in the stage, the stage must be set before
	 * calling this method.
	 * 
	 * @param capture
	 *            If true, the capture listeners will be notified instead of the
	 *            regular listeners.
	 ...
	 * {@link #getTouchable() touchable} and {@link #isVisible() visible}, or
	 * null if no actor was hit. The point is specified in the actor's local
	 * coordinate system (0,0 is the bottom left of the actor and width,height
	 * is the upper right).
	 * <p>
	 ...
	 * 
	 * @param touchable
	 *            If true, the hit detection will respect the
	 ...
	 * hit} this actor. See {@link #fire(Event)}.
	 * 
	 * @see InputListener
	 * @see ClickListener
	 */
	...
	/**
	 * Returns the stage that this actor is currently in, or null if not in a
	 * stage.
	 */
	 ...
	 * 
	 * @param stage
	 *            May be null if the actor or any parent is no longer in a
	 *            stage.
	 */
	...
	/**
	 * Returns this actor or the first ascendant of this actor that is
	 * assignable with the specified type.
	 */
	...
	/**
	 * Called by the framework when an actor is added to or removed from a
	 * group.
	 * 
	 * @param parent
	...
	/**
	 * Sets the origin position which is relative to the actor's bottom left
	 * corner.
	 */
	 ...
	 * parent's {@link Group#getChildren() children}, where a lower index is
	 * below a higher index. Setting a z-index higher than the number of
	 * children will move the child to the front. Setting a z-index less than
	 * zero is invalid.
	 */
	 ...
	 * transform matrix of the stage's Batch. The transform matrix and the
	 * stage's camera must not have rotational components. Calling this method
	...
	/**
	 * Draws a rectange for the bounds of this actor if {@link #getDebug()} is
	 * true.
	 */

...
public class Event implements Poolable {
	...
	private boolean capture; // true means event occurred during the capture
								// phase
	private boolean bubbles = true; // true means propagate to target's parents
	private boolean handled; // true means the event was handled (the stage will
								// eat the input)
	...
	private boolean cancelled; // true means propagation was stopped and any
								// action that this event would cause should not
								// happen

	/**
	 * Marks this event as handled. This does not affect event propagation
	 * inside scene2d, but causes the {@link Stage} event methods to return
	 * true, which will eat the event so it is not passed on to the application
	 * under the stage.
	 */
	public void handle() {
		handled = true;
	 ...
	 * action that would have been taken by the code that fired the event. Eg,
	 * if the event is for a checkbox being checked, cancelling the event could
	...
	/**
	 * Marks this event has being stopped. This halts event propagation. Any
	 * other listeners on the {@link #getListenerActor() listener actor} are
	 * notified, but after that no other listeners are notified.
	 */
	public void stop() {
		stopped = true;
	}
	...
	/**
	 * If true, after the event is fired on the target actor, it will also be
	 * fired on each of the parent actors, all the way to the root.
	 */

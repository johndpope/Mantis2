...
public class Tree extends WidgetGroup {
	...
	private void draw(Batch batch, Array<Node> nodes, float indent) {
		...
		for (int i = 0, n = nodes.size; i < n; i++) {
			...
			expandIcon.draw(batch, x + indent - iconSpacingLeft, y + iconY, expandIcon.getMinWidth(),
	...
	/**
	 * Returns the click listener the tree uses for clicking on nodes and the
	 * over node.
	 */
	...
	static public class Node {
		...
		/**
		 * Called to add the actor to the tree when the node's parent is
		 * expanded.
		 */
		...
		/**
		 * Called to remove the actor from the tree when the node's parent is
		 * collapsed.
		 */
		...
		/**
		 * If the children order is changed, {@link #updateChildren()} must be
		 * called.
		 */
		...
		/**
		 * Returns this node or the child node with the specified object, or
		 * null.
		 */
		public Node findNode(Object object) {
			if (object == null)

...
public class DragAndDrop {
	...
	public void addSource(final Source source) {
		DragListener listener = new DragListener() {
			...
			public void drag(InputEvent event, float x, float y, int pointer) {
				...
				Actor hit = event.getStage().hit(stageX, stageY, true); // Prefer
																		// touchable
																		// actors.
				if (hit == null)
				...
				// If over a new target, notify the former target that it's
				// being left behind.
				if (newTarget != target) {
					if (target != null)
	...
	static abstract public class Source {
		...
		/**
		 * Called when a drag is started on the source. The coordinates are in
		 * the source's local coordinate system.
		 * 
		...
		/**
		 * Called when a drag for the source is stopped. The coordinates are in
		 * the source's local coordinate system.
		 * 
		 * @param payload
	...
	static abstract public class Target {
		...
		/**
		 * Called when the payload is no longer over the target, whether because
		...
		/**
		 * Called when the payload is dropped on the target. The coordinates are
	 ...
	 * provided to follow the cursor and change when over a target. Such Actors
	 * will be added and removed from the stage automatically during the drag
	 * operation. Care should be taken when using the source Actor as a payload
	 * drag actor.
	 */

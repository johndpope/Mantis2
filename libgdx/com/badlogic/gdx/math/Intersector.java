...
public final class Intersector {
	...
	/**
	 * Determines on which side of the given line the point is. Returns -1 if
	 * the point is on the left side of the line, 0 if the point is on the line
	 * and 1 if the point is on the right side of the line. Left and right are
	 * relative to the lines direction which is linePoint1 to linePoint2.
	 */
	 ...
	 * overlap polygon resulting from the intersection. Follows the
	...
	/**
	 * Checks whether the line segment and the circle intersect and returns by
	 ...
	 * the intersection point. You can get the intersection point by: Vector2
	 ...
	 * happened, the return value is in the range [0,1]. The intersection point
	 * can be recovered by point1 + t * (point2 - point1) where t is the return
	 ...
	 * 
	 * @param ray
	 *            The ray, the direction component must be normalized before
	 *            calling this method
	 * @param center
	 ...
	 * point on the ray closest to the origin which is within the specified
	 * bounds.
	 * 
	 * <p>
	 * The returned intersection (if any) is guaranteed to be within the bounds
	 * of the bounding box, but it can occasionally diverge slightly from ray,
	 * due to small floating-point errors.
	 * </p>
	 * 
	 * <p>
	 * If the origin of the ray is inside the box, this method returns true and
	 * the intersection point is set to the origin of the ray, accordingly to
	...
	/**
	 * Determines whether the given rectangles intersect and, if they do, sets
	 * the supplied {@code intersection} rectangle to the area of overlap.
	 * 
	 ...
	 *            The second polygon.
	 * @param mtv
	 *            A Minimum Translation Vector to fill in the case of a
	 ...
	 * vertex arrays overlap. If they do, optionally obtain a Minimum
	 * Translation Vector indicating the minimum magnitude vector required to
	 * push the polygon defined by verts1 out of the collision with the polygon
	 * defined by verts2.
	 * 
	 * @param verts1
	...
	public static boolean overlapConvexPolygons(float[] verts1, int offset1, int count1, float[] verts2, int offset2,
		...
		for (int i = offset1; i < end1; i += 2) {
			...
			for (int j = offset2; j < end2; j += 2) {
				// Counts the number of points that are within the projected
				// area.
	 ...
	 * SplitTriangle instance. Depending on where the triangle is relative to
	 * the plane, the result can be:
	 * 
	 * <ul>
	 ...
	 * {@link SplitTriangle#front} contains 2 triangles,
	 * {@link SplitTriangle#back} contains 1 triangles,
	 * {@link SplitTriangle#total} will be 3.</li>
	 * <li>Triangle has one vertex in front, two behind:
	 * {@link SplitTriangle#front} contains 1 triangle,
	 * {@link SplitTriangle#back} contains 2 triangles,
	 * {@link SplitTriangle#total} will be 3.</li>
	 * </ul>
	 * 
	 * The input triangle should have the form: x, y, z, x2, y2, z2, x3, y3, z3.
	 * One can add additional attributes per vertex which will be interpolated
	 * if split, such as texture coordinates or normals. Note that these
	 * additional attributes won't be normalized, as might be necessary in case
	 * of normals.
	 * 
	 * @param triangle
	 * @param plane
	...
	public static void splitTriangle(float[] triangle, Plane plane, SplitTriangle split) {
		...
		if (r1 != r2) {
			...
			// flip side and add new vertex and second edge vertex to current
			// side

...
public class Quaternion implements Serializable {
	...
	public Quaternion setEulerAnglesRad(float yaw, float pitch, float roll) {
		...
		x = (chy_shp * chr) + (shy_chp * shr); // cos(yaw/2) * sin(pitch/2) *
												// cos(roll/2) + sin(yaw/2) *
												// cos(pitch/2) * sin(roll/2)
		y = (shy_chp * chr) - (chy_shp * shr); // sin(yaw/2) * cos(pitch/2) *
												// cos(roll/2) - cos(yaw/2) *
												// sin(pitch/2) * sin(roll/2)
		z = (chy_chp * shr) - (shy_shp * chr); // cos(yaw/2) * cos(pitch/2) *
												// sin(roll/2) - sin(yaw/2) *
												// sin(pitch/2) * cos(roll/2)
		w = (chy_chp * chr) + (shy_shp * shr); // cos(yaw/2) * cos(pitch/2) *
												// cos(roll/2) + sin(yaw/2) *
												// sin(pitch/2) * sin(roll/2)
		return this;
	}
	...
	/**
	 * Sets the Quaternion from the given matrix, optionally removing any
	 * scaling.
	 */
	...
	/**
	 * Sets the Quaternion from the given rotation matrix, which must not
	 * contain scaling.
	 */
	...
	/**
	 * Get the axis angle representation of the rotation in degrees. The
	 * supplied vector will receive the axis (x, y and z values) of the rotation
	 * and the value returned is the angle in degrees around that axis. Note
	 * that this method will alter the supplied vector, the existing value of
	 * the vector is ignored.
	 * </p>
	 * This will normalize this quaternion if needed. The received axis is a
	 * unit vector. However, if this is an identity quaternion (no rotation),
	 * then the length of the axis may be zero.
	 * 
	 * @param axis
	...
	/**
	 * Get the axis-angle representation of the rotation in radians. The
	 * supplied vector will receive the axis (x, y and z values) of the rotation
	 * and the value returned is the angle in radians around that axis. Note
	 * that this method will alter the supplied vector, the existing value of
	 * the vector is ignored.
	 * </p>
	 * This will normalize this quaternion if needed. The received axis is a
	...
	public float getAxisAngleRad(Vector3 axis) {
		if (this.w > 1)
			this.nor(); // if w>1 acos and sqrt will produce errors, this cant
						// happen if quaternion is normalised
		float angle = (float) (2.0 * Math.acos(this.w));
		double s = Math.sqrt(1 - this.w * this.w); // assuming quaternion
													// normalised then w is less
													// than 1, so term always
													// positive.
		if (s < MathUtils.FLOAT_ROUNDING_ERROR) { // test to avoid divide by
													// zero, s is always
													// positive due to sqrt
			// if s close to zero then direction of axis not important
			axis.x = this.x; // if it is important that axis is normalised then
	...
	/**
	 * Get the angle in radians of the rotation this quaternion represents. Does
	 * not normalize the quaternion. Use {@link #getAxisAngleRad(Vector3)} to
	 * get both the axis and the angle of this rotation. Use
	 * {@link #getAngleAroundRad(Vector3)} to get the angle around a specific
	 * axis.
	 * 
	 ...
	 * twist rotation represents the rotation around the specified axis. The
	 * swing rotation represents the rotation of the specified axis itself,
	 * which is the rotation around an axis perpendicular to the specified axis.
	 * </p>
	...
	/**
	 * Get the angle in radians of the rotation around the specified axis. The
	...
	/**
	 * Get the angle in degrees of the rotation around the specified axis. The
	 * axis must be normalized.
	 * 
	 * @param axisX
	 *            the x component of the normalized axis for which to get the

...
public class Frustum {
	protected static final Vector3[] clipSpacePlanePoints = { new Vector3(-1, -1, -1), new Vector3(1, -1, -1),
			...
			new Vector3(-1, -1, 1), new Vector3(1, -1, 1), new Vector3(1, 1, 1), new Vector3(-1, 1, 1) }; // far
																											// clip
	 ...
	 * projection and view matrix, e.g. from an {@link OrthographicCamera} or
	...
	// window coordinate system has it's y downwards. The
	// * returned Ray is a member of this instance so don't reuse it outside
	// this class.
	// *
	...
	// Vector3 near_point =
	// X.tmp3().mul(near_width).mul(n_x).add(Y.tmp2().mul(near_height).mul(n_y));

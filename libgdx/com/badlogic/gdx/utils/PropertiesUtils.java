...
public final class PropertiesUtils {
	...
	/**
	 * Adds to the specified {@code ObjectMap} the key/value pairs loaded from
	 * the {@code Reader} in a simple line-oriented format compatible with
	...
	public static void load(ObjectMap<String, String> properties, Reader reader) throws IOException {
		...
		while (true) {
			...
			if (mode == SLASH) {
				...
				// if (Character.isWhitespace(nextChar)) { <-- not supported by
				// GWT; replaced with isSpace.
	 ...
	 * <code>java.util.Properties</code>.
	 * <p>
	 * Every entry in the <code>ObjectMap</code> is written out, one per line.
	 * For each entry the key string is written, then an ASCII <code>=</code>,
	 * then the associated element string. For the key, all space characters are
	 * written with a preceding <code>\</code> character. For the element,
	 * leading space characters, but not embedded or trailing space characters,
	 * are written with a preceding <code>\</code> character. The key and
	 * element characters <code>#</code>, <code>!</code>, <code>=</code>, and

...
public class Timer {
	...
	/**
	 * Timer instance for general application wide usage. Static methods on
	 * {@link Timer} make convenient use of this instance.
	 */
	...
	/**
	 * Schedules a task to occur once as soon as possible, but not sooner than
	 ...
	 * change at any time. Synchronize on the timer instance to prevent tasks
	...
	static abstract public class Task implements Runnable {
		...
		public Task() {
			app = Gdx.app; // Need to store the app when the task was created
							// for multiple LwjglAWTCanvas.
			if (app == null)
		 ...
		 * by a timer. The execution time may be reached after calling this
		 * method which may change the scheduled state. To prevent the scheduled
		 * state from changing, synchronize on this task object, eg:
		 * 
		 * <pre>
		 * synchronized (task) {
		 * 	if (!task.isScheduled()) { ... }
	...
	/**
	 * Manages the single timer thread. Stops thread on libgdx application pause
	 * and dispose, starts thread on resume.
	 * 
	 * @author Nathan Sweet
	 */

...
public class Queue<T> implements Iterable<T> {
	/**
	 * Contains the values in the queue. Head and tail indices go in a circle
	 * around this array, wrapping at the end.
	 */
	protected T[] values;

	/**
	 * Index of first element. Logically smaller than tail. Unless empty, it
	 * points to a valid element inside queue.
	 */
	protected int head = 0;

	/**
	 * Index of last element. Logically bigger than head. Usually points to an
	 * empty position, but points to the head when full (size == values.length).
	 */
	...
	/**
	 * Creates a new Queue which can hold the specified number of values without
	 * needing to resize backing array. This creates backing array of the
	 * specified type via reflection, which is necessary only when accessing the
	...
	/**
	 * Append given object to the tail. (enqueue to tail) Unless backing array
	...
	/**
	 * Returns the index of first occurrence of value in the queue, or -1 if no
	 * such value exists.
	 * 
	 * @param identity
	 *            If true, == comparison will be used. If false, .equals()
	 *            comparison will be used.
	 * @return An index of first occurrence of value in queue or -1 if no such
	 *         value exists
	 */
	...
	/**
	 * Retrieves the value in queue without removing it. Indexing is from the
	 * front to back, zero based. Therefore get(0) is the same as
	 * {@link #first()}.
	 * 
	...
	/**
	 * Removes all values from this queue. Values in backing array are set to
	 * null to prevent memory leak, so this operates in O(n).
	 */
	public void clear() {
		if (size == 0)
			return;
	 ...
	 * called. Use the {@link QueueIterator} constructor for nested or
	...
	static public class QueueIterable<T> implements Iterable<T> {
		...
		public Iterator<T> iterator() {
			...
			// new Throwable().printStackTrace(new
			// java.io.PrintWriter(lastAcquire));
			if (iterator1 == null) {

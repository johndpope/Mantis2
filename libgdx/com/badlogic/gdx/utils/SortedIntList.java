...
public class SortedIntList<E> implements Iterable<SortedIntList.Node<E>> {
	...
	public E insert(int index, E value) {
		if (first != null) {
			Node<E> c = first;
			// iterate to the right until we can't move any further because the
			// next number is bigger than index
	...
	class Iterator implements java.util.Iterator<Node<E>> {
		...
		@Override
		public void remove() {
			// the contract specifies to remove the last returned element, if
			// nothing was returned yet assumably do nothing

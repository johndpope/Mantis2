...
public class Pixmap implements Disposable {
	...
	/**
	 * Sets the type of {@link Blending} to be used for all operations. Default
	 * is {@link Blending#SourceOver}.
	 * 
	 * @param blending
	 *            the blending type
	 */
	 ...
	 * {@link Pixmap#setFilter(Filter)} to specify the type of filtering to be
	 * used (nearest neighbour or bilinear).
	 * 
	 * @param pixmap
	 *            The other Pixmap
	 * @param srcx
	...
	/**
	 * Fills a triangle with vertices at x1,y1 and x2,y2 and x3,y3 using the
	 * current color.
	 * 
	 * @param x1
	...
	/**
	 * Returns the OpenGL ES format of this Pixmap. Used as the third parameter
	 * to
	 * {@link GL20#glTexImage2D(int, int, int, int, int, int, int, int, java.nio.Buffer)}.
	 * 
	 * @return one of GL_ALPHA, GL_RGB, GL_RGBA, GL_LUMINANCE, or
	...
	/**
	 * Returns the OpenGL ES type of this Pixmap. Used as the eighth parameter
	 * to
	 * {@link GL20#glTexImage2D(int, int, int, int, int, int, int, int, java.nio.Buffer)}.
	 * 
	 ...
	 * Alpha each value is encoded as a byte. For the format LuminanceAlpha the
	 * luminance is the first byte and the alpha is the second byte of the
	 * pixel. For the formats RGB888 and RGBA8888 the color components are
	 * stored in a single byte each in the order red, green, blue (alpha). For
	 * the formats RGB565 and RGBA4444 the pixel colors are stored in shorts in

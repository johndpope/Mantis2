...
public abstract class Camera {
	...
	/**
	 * Recalculates the projection and view matrix of this camera and the
	 * {@link Frustum} planes if <code>updateFrustum</code> is true. Use this
	 * after you've manipulated any of the attributes of the camera.
	 */
	...
	public void lookAt(float x, float y, float z) {
		...
		if (!tmpVec.isZero()) {
			float dot = tmpVec.dot(up); // up and direction must ALWAYS be
										// orthonormal vectors
	 ...
	 * around the given axis. The direction and up vector will not be
	 * orthogonalized.
	 * 
	 * @param angle
	 *            the angle
	 ...
	 * It's the same as GLU gluUnProject, but does not rely on OpenGL. The x-
	 * and y-coordinate of vec are assumed to be in screen coordinates (origin
	 * is the top left corner, y pointing down, x pointing to the right) as
	 * reported by the touch methods in {@link Input}. A z-coordinate of 0 will
	 * return a point on the near plane, a z-coordinate of 1 will return a point
	 * on the far plane. This method allows you to specify the viewport position
	 * and dimensions in the coordinate system expected by
	 * {@link GL20#glViewport(int, int, int, int)}, with the origin in the
	 * bottom left corner of the screen.
	 * 
	 * @param screenCoords
	 *            the point in screen coordinates (origin top left)
	 * @param viewportX
	 *            the coordinate of the bottom left corner of the viewport in
	...
	/**
	 * Function to translate a point given in screen coordinates to world space.
	 * It's the same as GLU gluUnProject but does not rely on OpenGL. The
	 * viewport is assumed to span the whole screen and is fetched from
	 * {@link Graphics#getWidth()} and {@link Graphics#getHeight()}. The x- and
	 * y-coordinate of vec are assumed to be in screen coordinates (origin is
	 * the top left corner, y pointing down, x pointing to the right) as
	 * reported by the touch methods in {@link Input}. A z-coordinate of 0 will
	 * return a point on the near plane, a z-coordinate of 1 will return a point
	 * on the far plane.
	 * 
	 ...
	 * It's the same as GLU gluProject with one small deviation: The viewport is
	 * assumed to span the whole screen. The screen coordinate system has its
	 * origin in the <b>bottom</b> left, with the y-axis pointing <b>upwards</b>
	 * and the x-axis pointing to the right. This makes it easily useable in
	 * conjunction with {@link Batch} and similar classes.
	 * 
	 ...
	 * origin in the <b>bottom</b> left, with the y-axis pointing <b>upwards</b>
	 * and the x-axis pointing to the right. This makes it easily useable in
	 * conjunction with {@link Batch} and similar classes. This method allows
	 * you to specify the viewport position and dimensions in the coordinate
	 * system expected by {@link GL20#glViewport(int, int, int, int)}, with the
	 ...
	 * screen coordinates origin is assumed to be in the top left corner, its
	 * y-axis pointing down, the x-axis pointing to the right. The returned
	 * instance is not a new instance but an internal member only accessible via
	 * this function.
	 * 
	 * @param viewportX
	 *            the coordinate of the bottom left corner of the viewport in

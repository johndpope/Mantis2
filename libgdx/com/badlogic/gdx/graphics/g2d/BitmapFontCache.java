...
public class BitmapFontCache {
	...
	/**
	 * Used internally to ensure a correct capacity for multi-page font vertex
	 * data.
	 */
	...
	/**
	 * Sets the alpha component of all text currently in the cache. Does not
	 * affect subsequently added text.
	 */
	 ...
	 * {@link #setText(CharSequence, float, float)} and is reset every time
	 * setText is called.
	 */
	...
	public void setColors(float color, int start, int end) {
		...
		for (int i = 0; i < pageCount; i++) {
			...
			// Loop through the indices and determine whether the glyph is
			// inside begin/end.
	 ...
	 * affects text subsequently added to the cache, but does not affect
	 * existing text currently in the cache.
	 */
	...
	/**
	 * A convenience method for setting the cache color. The color can also be
	...
	public void draw(Batch spriteBatch, int start, int end) {
		...
		// Determine vertex offset and count to render for each page. Some pages
		// might not need to be rendered at all.
		Array<TextureRegion> regions = font.getRegions();
		for (int i = 0, pageCount = pageVertices.length; i < pageCount; i++) {
			int offset = -1, count = 0;

			// For each set of glyph indices, determine where to begin within
			...
			for (int ii = 0, n = glyphIndices.size; ii < n; ii++) {
				...
				// Determine if this glyph is within bounds. Use the first match
				// of that for the offset.
	...
	private void addToCache(GlyphLayout layout, float x, float y) {
		...
		currentTint = whiteTint; // Cached glyphs have changed, reset the
									// current tint.
	}
	 ...
	 *            (the {@link BitmapFontData#capHeight cap height}).
	 * @param start
	 *            The first character of the string to draw.
	 * @param end
	 *            The last character of the string to draw (exclusive).
	 * @param targetWidth
	 *            The width of the area the text will be drawn, for wrapping or
	 *            truncation.
	 * @param halign
	 *            Horizontal alignment of the text, see {@link Align}.
	 * @param wrap
	 *            If true, the text will be wrapped within targetWidth.
	 * @param truncate
	 *            If not null, the text will be truncated within targetWidth
	 *            with this string appended. May be an empty string.
	 * @return The glyph layout for the cached string (the layout's height is
	 *         the distance from y to the baseline).
	 */
	public GlyphLayout addText(CharSequence str, float x, float y, int start, int end, float targetWidth, int halign,
			boolean wrap, String truncate) {
	...
	/**
	 * Returns the x position of the cached string, relative to the position
	 * when the string was cached.
	 */
	public float getX() {
		return x;
	}
	...
	/**
	 * Returns the y position of the cached string, relative to the position
	 * when the string was cached.
	 */
	public float getY() {
		return y;
	...
	/**
	 * Specifies whether to use integer positions or not. Default is to use them
	 * so filtering doesn't kick in as badly.
	 * 
	 * @param use
	 */

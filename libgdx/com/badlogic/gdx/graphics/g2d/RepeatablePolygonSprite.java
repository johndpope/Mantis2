...
public class RepeatablePolygonSprite {
		...
		for (int col = 0; col < cols; col++) {
			for (int row = 0; row < rows; row++) {
				...
				if (verts.length > 0) {
					...
					// adding null for key consistancy, needed to get col/row
					// from key
					// the other alternative is to make parts -
					// IntMap<FloatArray>
					parts.add(null);
				}
			}
	 ...
	 * then the grid values Snapping exactly to grid is important, so that
	 * during bulidVertices method, it can be figured out if points is on the
	 * wall of it's own grid box or not, to set u/v properly. Any other
	...
	/**
	 * Offsets polygon to 0 coordinate for ease of calculations, later offset is
	 * put back on final render
	 * 
	 * @param vertices
	...
	/**
	 * @param color
	 *            - Tint color to be applied to entire polygon
	 */

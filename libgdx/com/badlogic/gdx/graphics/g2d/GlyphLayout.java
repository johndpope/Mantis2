...
public class GlyphLayout implements Poolable {
	 ...
	 * setText} with the whole string, the font's current color, and no
	...
	/**
	 * @param color
	 *            The default color to use for the text (the BitmapFont
	 *            {@link BitmapFont#getColor() color} is not used). If
	 *            {@link BitmapFontData#markupEnabled} is true, color markup
	 *            tags in the specified string may change the color for portions
	 *            of the text.
	 * @param halign
	 *            Horizontal alignment of the text, see {@link Align}.
	 * @param targetWidth
	 *            The width used for alignment, line wrapping, and truncation.
	 *            May be zero if those features are not used.
	 * @param truncate
	 *            If not null and the width of the glyphs exceed targetWidth,
	 *            the glyphs are truncated and the glyphs for the specified
	 *            truncate string are placed at the end. Empty string can be
	 *            used to truncate without adding glyphs. Truncate should not be
	 *            used with text that contains multiple lines. Wrap is ignored
	...
	public void setText(BitmapFont font, CharSequence str, int start, int end, Color color, float targetWidth,
			int halign, boolean wrap, String truncate) {
		if (truncate != null)
			wrap = true; // Causes truncate code to run, doesn't actually cause
							// wrapping.
		else if (targetWidth <= font.data.spaceWidth) //
			wrap = false; // Avoid one line per character, which is very
							// inefficient.
		...
		outer: while (true) {
			...
			if (runEnd != -1) {
				if (runEnd != runStart) { // Can happen (eg) when a color tag is
											// at text start or a line is "\n".
					...
					if (run.glyphs.size == 0)
						...
						// Compute the run width, wrap if necessary, and
						// position the run.
						...
						for (int i = 0, n = run.xAdvances.size; i < n; i++) {
							...
							// Don't wrap if the glyph would fit with just its
							// width (no xadvance or kerning).
							if (wrap && x > targetWidth && i > 1 && x - xAdvance
									+ (run.glyphs.get(i - 1).xoffset + run.glyphs.get(i - 1).width) * fontData.scaleX
									- 0.0001f > targetWidth) {
								...
								if ((run.x == 0 && wrapIndex == 0) // Require at
																	// least one
																	// glyph per
																	// line.
										|| wrapIndex >= run.glyphs.size) { // Wrap
																			// at
																			// least
																			// the
																			// glyph
								...
								GlyphRun next;
								if (wrapIndex == 0)
									next = run; // No wrap index, move entire
												// run to next line.
								else {
								...
								// Start the loop over with the new run on the
								// next line.
		...
		if ((halign & Align.left) == 0) { // Not left aligned, so must be center
											// or right aligned.
		...
		this.height = fontData.capHeight + lines * fontData.lineHeight
				+ blankLines * fontData.lineHeight * fontData.blankLineScale;
	}
	...
	private GlyphRun wrap(BitmapFontData fontData, GlyphRun first, Pool<GlyphRun> glyphRunPool, int wrapIndex,
		...
		// Reduce first run width by the wrapped glyphs that have contributed to
		// the width.
		while (widthIndex > wrapIndex + 1)
			first.width -= first.xAdvances.get(--widthIndex);

		// Copy wrapped glyphs and xAdvances to second run.
		// The second run will contain the remaining glyph data, so swap
		// instances rather than copying to reduce large allocations.
		if (wrapIndex < glyphCount) {
			...
			// Equivalent to:
			// second.glyphs.addAll(first.glyphs, wrapIndex, glyphCount -
			// wrapIndex);
			// first.glyphs.truncate(wrapIndex);

			FloatArray xAdvances1 = second.xAdvances; // Starts empty.
			FloatArray xAdvances2 = first.xAdvances; // Starts with all the
														// xAdvances.
			xAdvances1.addAll(xAdvances2, 0, wrapIndex + 1);
			xAdvances2.removeRange(1, wrapIndex); // Leave first entry to be
													// overwritten by next line.
			xAdvances2.set(0, -glyphs2.first().xoffset * fontData.scaleX - fontData.padLeft);
			first.xAdvances = xAdvances1;
			second.xAdvances = xAdvances2;
			// Equivalent to:
			// second.xAdvances.add(-second.glyphs.first().xoffset *
			// fontData.scaleX - fontData.padLeft);
			// second.xAdvances.addAll(first.xAdvances, wrapIndex + 1,
			// first.xAdvances.size - (wrapIndex + 1));
	...
	/**
	 * Adjusts the xadvance of the last glyph to use its width instead of
	 * xadvance.
	 */
	private void adjustLastGlyph(BitmapFontData fontData, GlyphRun run) {
		...
		if (fontData.isWhitespace((char) last.id))
			return; // Can happen when doing truncate.
		float width = (last.xoffset + last.width) * fontData.scaleX - fontData.padRight;
		run.width += width - run.xAdvances.peek(); // Can cause the run width to
													// be > targetWidth, but the
													// problem is minimal.
	...
	private int parseColorMarkup(CharSequence str, int start, int end, Pool<Color> colorPool) {
		...
		switch (str.charAt(start)) {
		case '#':
			// Parse hex color RRGGBBAA where AA is optional and defaults to
			// 0xFF if less than 6 chars are used.
			int colorInt = 0;
	...
	/**
	 * Stores glyphs and positions for a piece of text which is a single color
	 * and does not span multiple lines.
	 * 
	 * @author Nathan Sweet
	 */
	static public class GlyphRun implements Poolable {
		...
		/**
		 * Contains glyphs.size+1 entries: First entry is X offset relative to
		 * the drawing position. Subsequent entries are the X advance relative
		 * to previous glyph position. Last entry is the width of the last
		 * glyph.
		 */

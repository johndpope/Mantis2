 ...
 * 
 * <pre>
 * // 512x512 pixel pages, RGB565 format, 2 pixels of padding, border
 * // duplication
 ...
 * 
 * <pre>
 * // 512x512 pixel pages, RGB565 format, 2 pixels of padding, no border
 * // duplication
 ...
 * 
 * // once the atlas is no longer needed, make sure you get the final additions.
 * // This might
...
public class PixmapPacker implements Disposable {
	 ...
	 * 
	 * @see PixmapPacker#PixmapPacker(int, int, Format, int, boolean,
	 *      PackStrategy)
	 */
	...
	/**
	 * Creates a new ImagePacker which will insert all supplied pixmaps into one
	 * or more <code>pageWidth</code> by <code>pageHeight</code> pixmaps using
	 * the specified strategy.
	 * 
	 * @param padding
	 *            the number of blank pixels to insert between pixmaps.
	 * @param duplicateBorder
	 *            duplicate the border pixels of the inserted images to avoid
	...
	/**
	 * Sorts the images to the optimzal order they should be packed. Some
	 * packing strategies rely heavily on the images being sorted.
	 */
	...
	/**
	 * Inserts the pixmap. If name was not null, you can later retrieve the
	 * image's position in the output image via {@link #getRect(String)}.
	 * 
	 * @param name
	 *            If null, the image cannot be looked up by name.
	 * @return Rectangle describing the area the pixmap was rendered to.
	 * @throws GdxRuntimeException
	 *             in case the image did not fit due to the page size being too
	 *             small or providing a duplicate name.
	 */
	...
	/**
	 * @return the {@link Page} instances created so far. If multiple threads
	 *         are accessing the packer, iterating over the pages must be done
	 *         only after synchronizing on the packer.
	 */
	...
	/**
	 * Disposes any pixmap pages which don't have a texture. Page pixmaps that
	 * have a texture will not be disposed until their texture is disposed.
	 */
	...
	/**
	 * Generates a new {@link TextureAtlas} from the pixmaps inserted so far.
	 * After calling this method, disposing the packer will no longer dispose
	 * the page pixmaps.
	 */
	 ...
	 * packed since the last call to this method. This can be used to insert
	 * Pixmap instances on a separate thread via {@link #pack(String, Pixmap)}
	...
	/**
	 * If true, when a pixmap is packed to a page that has a texture, the
	 * portion of the texture where the pixmap was packed is updated using
	 * glTexSubImage2D. Note if packing many pixmaps, this may be slower than
	 * reuploading the whole texture. This setting is ignored if
	 * {@link #getDuplicateBorder()} is true.
	 */
	...
	static public class Page {
		...
		/**
		 * Returns the texture for this page, or null if the texture has not
		 * been created.
		 * 
	...
	static public interface PackStrategy {
		...
		/**
		 * Returns the page the rectangle should be placed in and modifies the
		 * specified rectangle position.
		 */
	...
	/**
	 * Does bin packing by inserting in rows. This is good at packing images
	...
	/**
	 * Sets the default <code>color</code> of the whole
	 * {@link PixmapPacker.Page} when a new one created. Helps to avoid texture

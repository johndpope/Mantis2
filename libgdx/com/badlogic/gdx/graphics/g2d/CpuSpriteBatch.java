...
public class CpuSpriteBatch extends SpriteBatch {
	 ...
	 * transform matrix is not {@link #setTransformMatrix(Matrix4) changed}.
	 * </p>
	 * <p>
	 * Note: The real transform matrix <em>must</em> be invertible. If a
	 * singular matrix is detected, GdxRuntimeException will be thrown.
	 * </p>
	 * 
	...
	/**
	 * Sets the transform matrix to be used by this Batch. Even if this is
	 * called inside a {@link #begin()}/{@link #end()} block, the current batch
	 * is <em>not</em> flushed to the GPU. Instead, for every subsequent draw()
	 * the vertices will be transformed on the CPU to match the original batch
	 * matrix. This adjustment must be performed until the matrices are
	 * realigned by restoring the original matrix, or by calling

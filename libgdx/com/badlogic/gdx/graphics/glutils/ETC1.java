...
public class ETC1 {
	...
	public final static class ETC1Data implements Disposable {
		...
		/**
		 * the offset in bytes to the actual compressed data. Might be 16 if
		 * this contains a PKM header, 0 otherwise
		 **/
	...
	/**
	 * Encodes the image via the ETC1 compression scheme. Only
	 * {@link Format#RGB565} and {@link Format#RGB888} are supported.
	 * 
	 * @param pixmap
	 *            the {@link Pixmap}
	...
	/**
	 * Encodes the image via the ETC1 compression scheme. Only
	 * {@link Format#RGB565} and {@link Format#RGB888} are supported. Adds a PKM
	 * header in front of the compressed image data.
	 * 
	 * @param pixmap
	 *            the {@link Pixmap}
	 * @return the {@link ETC1Data}
	 ...
	 * {@link Format#RGB565} or {@link Format#RGB888} {@link Pixmap}. Does not
	 * modify the ByteBuffer's position or limit.
	 * 
	 * @param etc1Data
	...
	public static native void formatHeader(ByteBuffer header, int offset, int width,
			int height); /*
							 * etc1_pkm_format_header((etc1_byte*)header +
							 * offset, width, height);
							 */
	 ...
	 * @param offset
	 *            the offset in bytes to the PKM header from the ByteBuffer's
	 *            start
	 * @return the width stored in the PKM header
	 */
	static native int getWidthPKM(ByteBuffer header,
			int offset); /*
							 * return etc1_pkm_get_width((etc1_byte*)header +
							 * offset);
							 */
	...
	static native int getHeightPKM(ByteBuffer header,
			int offset); /*
							 * return etc1_pkm_get_height((etc1_byte*)header +
							 * offset);
							 */
	...
	static native boolean isValidPKM(ByteBuffer header,
			int offset); /*
							 * return etc1_pkm_is_valid((etc1_byte*)header +
							 * offset) != 0?true:false;
							 */

	/**
	 * Decodes the compressed image data to RGB565 or RGB888 pixel data. Does
	 * not modify the position or limit of the {@link ByteBuffer} instances.
	 * 
	 * @param compressedData
	 *            the compressed image data in a direct native order
	 *            {@link ByteBuffer}
	 * @param offset
	 *            the offset in bytes to the image data from the start of the
	 *            buffer
	 * @param decodedData
	 *            the decoded data in a direct native order ByteBuffer, must
	 *            hold width * height * pixelSize bytes.
	 * @param offsetDec
	...
	private static native void decodeImage(ByteBuffer compressedData, int offset, ByteBuffer decodedData, int offsetDec,
			...
			int pixelSize); /*
							 * etc1_decode_image((etc1_byte*)compressedData +
							 * offset, (etc1_byte*)decodedData + offsetDec,
	 ...
	 * @return a new direct native order ByteBuffer containing the compressed
	 *         image data
	 */
	private static native ByteBuffer encodeImage(ByteBuffer imageData, int offset, int width, int height,
			int pixelSize); /*
							 ...
							 * etc1_byte* compressedData =
							 * (etc1_byte*)malloc(compressedSize);
							 * etc1_encode_image((etc1_byte*)imageData + offset,
							 * width, height, pixelSize, width * pixelSize,
							 * compressedData); return
							 * env->NewDirectByteBuffer(compressedData,
							 * compressedSize);
							 */
	...
	private static native ByteBuffer encodeImagePKM(ByteBuffer imageData, int offset, int width, int height,
			int pixelSize); /*
							 * int compressedSize =
							 * etc1_get_encoded_data_size(width, height);
							 * etc1_byte* compressed =
							 * (etc1_byte*)malloc(compressedSize +
							 * ETC_PKM_HEADER_SIZE);
							 * etc1_pkm_format_header(compressed, width,
							 * height); etc1_encode_image((etc1_byte*)imageData
							 * + offset, width, height, pixelSize, width *
							 * pixelSize, compressed + ETC_PKM_HEADER_SIZE);
							 * return env->NewDirectByteBuffer(compressed,

...
public class Texture extends GLTexture {
	 ...
	 * then calls {@link #load(TextureData)}. Use this only if you know what you
	 * do!
	 */
	@Override
	 ...
	 * clipping is performed so you have to make sure that you draw only inside
	...
	public void dispose() {
		...
		// dispose) the texture
		// and then reload it. the glHandle is set to 0 in invalidateAllTextures
		// prior to
	...
	/**
	 * Invalidate all managed textures. This is an internal method. Do not use
	 * it!
	 */
	public static void invalidateAllTextures(Application app) {
		...
		if (assetManager == null) {
			...
			// first we have to make sure the AssetManager isn't loading
			// anything anymore,
			// otherwise the ref counting trick below wouldn't work (when a
			// texture is
			// currently on the task stack of the manager.)
			assetManager.finishLoading();

			// next we go through each texture and reload either directly or via
			// the
			// asset manager.
			...
			for (Texture texture : textures) {
				...
				if (fileName == null) {
					...
					// can actually remove it from the assetmanager. Also set
					// the
					...
					// create the parameters, passing the reference to the
					// texture as
					...
					params.genMipMaps = texture.data.useMipMaps(); // not sure
																	// about
																	// this?
					...
					// unload the texture, create a new gl handle then reload
					// it.
	 ...
	 * by the asset manager are reloaded by the manager on a separate thread
	 * (provided that a suitable {@link AssetLoader} is registered with the
	 * manager). Textures not managed by the AssetManager are reloaded via the

...
public class MUILabelTypeAnim implements MAnimation {
	...
	public MUILabelTypeAnim(String name, String typeText, float len, boolean loopAnim, MCallback cb, MUIPane pane) {
		object = name;
		length = len;
		loop = loopAnim;
		text = typeText;
	...
	public boolean tick(float delta) {
		...
		if (owningPane.doesUIElementExist(object))
			owningPane.setLabelText(object, text.substring(0, charI));
		else
			return true;
		if (reverse)
			elapsed -= delta;
